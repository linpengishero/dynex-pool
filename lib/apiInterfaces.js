...
function rpc_mallob (host, port, method, params, mallob, callback) {
	var data = JSON.stringify({
		id: "0",
		jsonrpc: "2.0",
		method: method,
...
module.exports = function (daemonConfig, walletConfig, poolApiConfig) {
	return {
		...
		rpcDaemon_mallob: function (method, params, mallob, callback, serverConfig) {
			if (serverConfig) {
				rpc_mallob(serverConfig.host, 9000, method, params, mallob, callback);
			} else {
				rpc_mallob(daemonConfig.host, 9000, method, params, mallob, callback);
			}
		},
