	...
	async.waterfall([
		...
		function (balances, minPayoutLevel, callback) {
			...
			for (let worker in payments) {
				...
				// log('info', logSystem, `address ${address}`);
				// log('info', logSystem, `addr ${addr}`);
				// log('info', logSystem, `addr[0] ${addr[0]}`);
				// log('info', logSystem, `addr[1] ${addr[1]}`);
				// log('info', logSystem, `addr.length ${addr.length}`);
				// log('info', logSystem, `utils.isIntegratedAddress(address) ${utils.isIntegratedAddress(address)}`);
				// if ((addr.length === 1 && utils.isIntegratedAddress(address)) || (addr.length >= 2 && addr[1] !== undefined)) {
				if (addr.length >= 2 && addr[1] !== undefined) {
			...
			async.filter(transferCommands, function (transferCmd, cback) {
				...
				// log('info', logSystem, `JSON.stringify(rpcRequest) ${JSON.stringify(rpcRequest)}`);
