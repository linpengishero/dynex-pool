...
let statValueHandler = {
	...
	avg2: function (set, value) {
		const oldValue = set[3] ? set[3] : 0;
		set[3] = (oldValue * set[2] + value) / (set[2] + 1);
	},
	avgRound2: function (set, value) {
		statValueHandler.avg2(set, value);
		set[3] = Math.round(set[3]);
	},
};
...
function storeCollectedValues (chartName, values, settings) {
	for (let i in values) {
		if (values[i]) {
			let dataToSave;
			dataToSave = values[i];
			storeCollectedValue(chartName + ':' + i, [dataToSave], settings);
		}
	}
}
	...
	values.forEach((value, index) => {
		...
		return getChartDataFromRedis(name, function (sets) {
			...
			if (!lastSet || now - lastSet[0] > settings.stepInterval) {
				lastSet = (value['hashrate'] && value['hashrateAvg']) ?
					[now, value['hashrate'], 1, value['hashrateAvg']] :
					[now, value, 1];
				sets.push(lastSet);
				...
				if (value['hashrate'] && value['hashrateAvg']) {
					// no preSaved functions for this
					statValueHandler.avgRound(lastSet, value['hashrate']);
					statValueHandler.avgRound2(lastSet, value['hashrateAvg']);
				} else {
...
function getUsersHashrates2 (callback) {
	apiInterfaces.pool('/miners_hashrate2', function (error, data) {
		if (error) {
			log('error', logSystem, 'Unable to get API data for miners_hashrate: ' + error);
		}
		let resultData = data && data.minersHashrate ? data.minersHashrate : {};
...
function collectUsersHashrate (chartName, settings) {
	...
	redisClient.keys(redisBaseKey + '*', function (keys) {
		...
		getUsersHashrates2(function (newHashrates) {
			// newHashrates2
			// {"XwnNBToTvFD2zrrB8vEhVEQMWJckoJZxTBhz4zhX9jHc4aCCALZmtNvUN8WET13XgZFtAx274tFw6DKTc3ASdr6P1NbE1NEqP":
			// 		{"hashrate":405100,"hashrateAvg":388929},
			// "Xwo2WNWT6PmjArxnPnUtW7bSFfhaXxFtyTQ7e5C3HBCtX1AnxK9kmUtV7aVrLGQJyGCzJ6R1BmyHzbDQbYTWGqXE1kicKbALX":
			// 		{"hashrate":163646,"hashrateAvg":177176},
			// "XwoGw8e2xpM9PAoqChDXEP8iYNwNVTQJ8NHoKna1bhhZ3pkLfqNTHKeTDgnM4X3qjsZrgfgojbnGEZRVepMAQYce28bbwxkrF":
			// 		{"hashrate":41504,"hashrateAvg":52765}
