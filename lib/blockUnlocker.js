...
function runInterval () {
	async.waterfall([
		...
		function (blocks, callback) {
			...
			let payments = {};
			let rewardsData = {};
			const transferFee = config.payments.transferFee ? config.payments.transferFee : 1000000;
			...
			blocks.forEach(function (block) {
				...
				if (block.workerScores) {
					...
					let rewardData = {
						type: block.rewardType,
						timestamp: now,
						hash: block.hash,
						height: block.height,
						roundShares: block.shares,
						difficulty: block.difficulty
					};
					...
					if (block.rewardType === 'solo') {
						...
						rewardData['reward'] = payments[worker];
						// rewardData['reward'] = payments[worker] - transferFee;
						rewardData['share'] = 1 - feePercent;
						rewardsData[worker] = rewardData;
						...
						Object.keys(block.workerScores).forEach(function (worker) {
							...
							rewardsData[worker] = {
								...rewardData,
								reward: payments[worker],
								share: percent * (1 - feePercent)
							};
							// reward: payments[worker] - transferFee,
			...
			for (let worker in rewardsData) {
				const rewardData = rewardsData[worker];
				let amount = parseInt(rewardData['reward']);
				if (amount <= 0) {
					continue;
				}

				unlockedBlocksCommands.push(['zadd', `${config.coin}:rewards:${rewardData['type']}:${worker}`, rewardData['timestamp'], [
					rewardData['reward'],	//	14990733
					rewardData['share'],	// 0.00008846143600444055
					rewardData['hash'],
					rewardData['height'],
					rewardData['roundShares'],
