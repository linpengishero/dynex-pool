...
function Miner (rewardType, childRewardType, id, childPoolIndex, login, pass, ip, port, agent, childLogin, startingDiff, noRetarget, pushMessage) {
	...
	this.shareTimeRing = utils.hashesBuffer(16);
...
Miner.prototype = {
	retarget: function (now) {
		...
		let avg = this.shareTimeRing.getAge(decreaser ? sinceLast : null);
		let newDiff;
		let direction;
	...
	getJob: function () {
		...
		//log('warn', logSystem, 'Retargetting difficulty for %d', [currentBlockTemplate.length]);
		let newJob = {
			id: utils.uid(),
...
function updateShareResult (miner, shareAccepted, job) {
	//log('warn', logSystem, 'updateShareResult');
	...
	if (shareAccepted) {
		//log('warn', logSystem, 'miner.hashesRing.append');
